# Лабораторная работа №3 (Backend): развёртывание edu-rust-gigachat-app-backend в Yandex Cloud
...
backend, загрузка образа в Yandex Container Registry и запуск в Yandex Cloud Serverless
Containers.

## 1. Цель работы

1. Развернуть учебное API‑приложение `edu-rust-gigachat-app-backend` как
...
4. Получить публичный URL контейнера и проверить работу API.

## 2. Что нужно заранее

- Аккаунт в Yandex Cloud с доступом к консоли.
- Установленный `yc` CLI.
- Установленный Docker (Docker Desktop для Windows/macOS).
- Доступ к репозиторию `edu-rust-gigachat-app-backend`.
...
## 7. Сборка backend и Docker‑образа

### 7.1. Сборка релизной версии
...
Сборка создаст бинарный файл `target/release/rust-gigachat-demo`.

### 7.2. Минимальный Dockerfile

Serverless Containers ожидают, что приложение слушает порт из переменной
окружения `PORT`. Для Rocket в этом проекте используется `APP_SERVER_PORT`
(через `APP_*`‑переопределение конфигурации), поэтому порт прокидывается
...
WORKDIR /app
COPY --from=builder /app/target/release/rust-gigachat-demo /app/rust-gigachat-demo

# Rocket должен слушать на 0.0.0.0, а порт берётся из PORT
ENV APP_SERVER_ADDRESS=0.0.0.0

CMD ["/bin/sh", "-c", "APP_SERVER_PORT=${PORT} /app/rust-gigachat-demo"]
```
...
```bash
docker build -t cr.yandex/<REGISTRY_ID>/edu-rust-gigachat-app-backend:latest .
```

2. Отправить образ:

```bash
docker push cr.yandex/<REGISTRY_ID>/edu-rust-gigachat-app-backend:latest
```
...
yc serverless container revision deploy \
  ...
  --image cr.yandex/<REGISTRY_ID>/edu-rust-gigachat-app-backend:latest \
  --cores 1 \
  --memory 512M \
...
## 11. Переменные окружения для backend

### 11.1. Базовые переменные

Минимально требуется:
- `APP_SERVER_ADDRESS=0.0.0.0`
- `APP_SERVER_PORT=<PORT>` (в Serverless Containers это значение даётся
  автоматически через переменную `PORT`)

### 11.2. GigaChat API

Для работы с реальным GigaChat необходимо задать:

- `GIGACHAT_TOKEN=<your_token>`

Если переменная не задана, backend перейдёт в mock‑режим.

**Важно:** добавление переменных окружения создаёт новую ревизию контейнера.
...
```bash
curl -H "Authorization: Bearer $(yc iam create-token)" https://<container-url>/health
```
...
4. Скриншот ответа `/health` или `/ask`.
...
- Режимы выполнения и переменная PORT: https://yandex.cloud/en/docs/serverless-containers/concepts/runtime
- Управление ревизиями (deploy): https://yandex.cloud/en/docs/serverless-containers/operations/manage-revision
...
- Создание реестра: https://yandex.cloud/en/docs/container-registry/operations/registry/registry-create
