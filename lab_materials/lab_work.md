# Лабораторная работа №1: Разработка веб-приложений на языке Rust
...
6.  **При использовании web-интерфейса обеспечить CORS:** если UI запущен на другом порту, браузер блокирует запросы к API без CORS-заголовков и обработки preflight (`OPTIONS`). Минимально требуется: `Access-Control-Allow-Origin`, `Access-Control-Allow-Methods: GET, POST, OPTIONS`, `Access-Control-Allow-Headers: Content-Type`. Подробное решение описано в документе `docs/cors_for_webapp.md`.
...
### 3.6. Требования по безопасности и подготовке материалов

Перед сдачей или публикацией результатов работы:

1. **Не коммитьте секреты**: удалите реальные токены и файлы `.env` из истории и рабочего каталога.
2. **Проверьте историю git** на наличие секретов:
   ```bash
   git log -p --all | grep -i "api-key\\|token\\|secret"
   ```
3. **Убедитесь, что mock‑режим работает**, если доступ к GigaChat недоступен.
4. **Проверьте сборку и тесты**: `cargo build` и `cargo test`.
...
В этой работе используется связка **beads + git** с рекомендованными настройками:

1. Установите hooks для автоматического связывания задач и коммитов:
   ```bash
   bd hooks install
   ```
2. Включите merge‑driver для `.beads/issues.jsonl` (чтобы избежать конфликтов):
   ```bash
   git config merge.beads.driver true
   ```
3. Используйте автоматическую синхронизацию beads (auto‑sync).

`bd sync` **по умолчанию выполняет `git pull` и `git push`**,
поэтому отдельный `git pull` до/после него не нужен.
